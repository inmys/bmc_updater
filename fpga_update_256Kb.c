...
uint8_t EEPROM_PageSizeCheck(int file_i2c, int chip_addr){
	int i, j;
	uint8_t bt;
	uint8_t t_buf[1024];
		
	for(i=0;i<10*PAGE_SIZE; i+=PAGE_SIZE) {
	...
	for(i=0;i<10*PAGE_SIZE; i+=PAGE_SIZE) {
		...
		for(j=0;j<PAGE_SIZE;j++) {
			if(t_buf[j] != file_buf[i+j])
				return 1; //PAGE SIZE must be changed
		}
	}

	return 0; //PAGE_SIZE OK
}
...
unsigned int EEPROM_GetHash(int file_i2c,int chip_addr){
	...
	EEPROM_PageRead(file_i2c,chip_addr,t_buf,i);
	uint32_t hash = crc32b(t_buf,PAGE_SIZE);
	return hash;
...
void EEPROM_Program(int chip_addr,int file_i2c, int sz){
	...
	printf("Test PAGE_SIZE...\n");
	PAGE_SIZE=64;
	if (EEPROM_PageSizeCheck(file_i2c,chip_addr) == 0){
		printf("PAGE_SIZE=64 is OK\n");
	} 
	else{
		printf("PAGE_SIZE changed to 128\n");
		PAGE_SIZE=128;
		if(EEPROM_PageSizeCheck(file_i2c,chip_addr) == 0){
			printf("PAGE_SIZE=128 is OK\n");
		}
	}

	printf("Programming...\n\r");
	for(i=0;i<sz+PAGE_SIZE; i+=PAGE_SIZE) {
